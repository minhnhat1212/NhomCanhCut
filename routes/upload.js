...
router.post('/excel', uploadExcel.single('file'), async function (req, res, next) {
    if (!req.file) {
        ...
        for (let index = 2; index <= worksheet.rowCount; index++) {
            ...
            let sku = cellValueToPlainString(element.getCell(1).value);
            let title = cellValueToPlainString(element.getCell(2).value);
            ...
            if (getTitle.includes(title)) {
                errorsInRow.push('title bi trung')
            }
